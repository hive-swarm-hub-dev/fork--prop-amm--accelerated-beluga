...
const MIN_FEE_BPS: u128 = 50;
const MAX_FEE_BPS: u128 = 400;
const BOOTSTRAP_FEE_BPS: u128 = 70;
const BOOTSTRAP_MIN_SAMPLES: u32 = 24;
const KAPPA_NUM: u128 = 3;
...
fn update_state(data: &[u8], storage: &mut [u8]) {
    ...
    let n = read_u32_le(&storage[16..20]);
    let last_step = read_u32_le(&storage[20..24]);

    // Step-gap gate: only update EWMA on first fill of each new step.
    let should_update_ewma = step > last_step as u64;
    ...
    // Always update log ratio and last_step
